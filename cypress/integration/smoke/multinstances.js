...
const AGENT_ID = "pet-multinstances";
...
context("Java agent smoke", () => {
  ...
  it("should display unregistered agent", () => {
    cy.get("table")
      ...
      .then(($link) => {
        expect($link.text()).to.eq(AGENT_ID);
  ...
  it("Register Java agent with only required parameters", () => {
    ...
    cy.get("input[name=id]").should("have.value", AGENT_ID);
    cy.get("input[name=name]").should("have.value", AGENT_ID);

    cy.get('[data-test="wizard:continue-button"]').click();
    cy.get('[data-test="wizard:continue-button"]').click();

    cy.intercept("PATCH", `/api/agents/${AGENT_ID}`, (req) => {
      req.continue((res) => {
  ...
  it("Check the coverage percentage when only one test session is finished", () => {
    cy.get('[data-test="name-column"]').contains(AGENT_ID).click();
    ...
    cy.task("startPetclinicAutoTests", { agentId: AGENT_ID }, { timeout: 200000 });
