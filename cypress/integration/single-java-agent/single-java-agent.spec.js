...
context("Test2Code part", () => {
  beforeEach(() => {
    cy.getByDataTest("name-column").contains(AGENT_NAME).click();
  ...
  //           // TODO need to decompose it because it look strange in log
  ...
  //               .find('[data-test="coverage-details:associated-tests-count"] a')
  //               .click();
  //           });
  //
  ...
  //     });
  //
  //     context("expanded entry", () => {
  //       it("should contain classes", () => {
  //         // TODO
  //       });
  //     });
  //
  //     context("double expanded entry", () => {
  ...
  //               .find('[data-test="test-actions:view-curl:id"] a')
  //               .click();
  //           });
  //
  ...
  context("Second build", () => {
    ...
    // });
    // TODO need to copy all tests for initial build with another fixtures with new methods
    ...
    //   context("Risks page", () => {
    //     beforeEach(() => {
    //       cy.getByDataTest("action-section:count:risks").click();
    //     });
    //
    ...
    //     beforeEach(() => {
    //       cy.getByDataTest("action-section:count:tests-to-run").click();
    //     });
    //
    ...
    //   //   cy.task("startPetclinicAutoTests", {}, { timeout: 200000 });
    //   //
    //   //   cy.get('[data-test="active-scope-info:finish-scope-button"]').click(); // TODo do it via api
    //   //   cy.get('[data-test="finish-scope-modal:finish-scope-button"]').click();
    //   // });
    //
    //   context("Overview page", () => {
    ...
    //               cy.contains("table tbody tr", riskName).find('[data-test="td-row-cell-type"]').should("have.text", riskData.type);
    //             });
    //
    //             it("should display coverage", () => {
    //               cy.contains("table tbody tr", riskName)
    //                 .find('[data-test="coverage-cell:coverage"]').should("contains", riskData.coverage);
    //             });
    //
    //             it("should display associated tests count", () => {
    //               cy.contains("table tbody tr", riskName)
    //                 .find('[data-test="risks-table:associated-tests-count"]')
    //                 .should("contains", riskData.associatedTestsCount);
    //             });
    //           });
    //         });
    //       });
    ...
    context("Tests to run", () => {
      ...
      context("Tests to run page", () => {
        beforeEach(() => {
          cy.contains('[data-test="action-section:count:tests-to-run"]', buildData.testsToRun.testsToRunCountAfterTheTestsExecuted).click();
        });
        ...
        context("tests to run table", () => {
          Object.entries(buildData.testsToRun.tests).map(([testName, testData]) => {
            context(`should display data for ${testName} test`, () => {
              ...
              it("should display tests type", () => {
                cy.contains("table tbody tr", testName).find('[data-test="td-row-type"]').should("have.text", testData.type);
              });
              ...
              it("should display methods covered", () => {
                cy.contains("table tbody tr", testName)
                  .find('[data-test="td-row-cell-coverage.methodCount.covered"]').should("have.text", testData.methodsCovered);
              });
            });
          });
        });
